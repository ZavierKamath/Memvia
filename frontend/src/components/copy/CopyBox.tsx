...
export function CopyboxElement(
	...
	return (
		<div className={`flex items-end justify-between bg-bg border-2 rounded-xl ${conditionalClass()}`}>
			...
			<button type="button"
				onClick={copyMessage}
				className="text-text-muted hover:text-text bg-bg-light px-2 py-2 rounded-xl hover:-motion-translate-y-loop-[10%] hover:motion-duration-700"
			>
				{buttonIcon}
			</button>
		</div>
...
export function Copybox() {
	...
	function conditionalRender() {
		if (active) {
			if (conditionalClass() === 'full') {
				return (
					<>
						<div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[10] rounded-xl bg-bg-dark border-2 border-primary text-text px-6 py-3 flex flex-col gap-4 items-center w-[56rem] h-[36rem] shadow-[0_0.25rem_0.5rem_rgba(0,0,0,0.2)] overflow-y-auto">
				...
				return (
					<div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[10] rounded-xl bg-bg-dark border-2 border-primary text-text-muted px-6 py-3 motion-translate-y-in-25 motion-opacity-in-0 motion-duration-300">
	...
	return (
		<>
			<button
				className={`rounded-xl px-4 py-2 bg-bg-light border-2  hover:text-text hover:border-primary shadow-[0_0.25rem_0.5rem_rgba(0,0,0,0.2)] hover:-motion-translate-y-loop-[10%] hover:motion-duration-700 ${active ? "text-text border-primary" : "border-border-muted text-text-muted"}`}
