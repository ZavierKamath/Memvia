...
export function CopyboxElement(
	...
	return (
		<div className={`flex items-end justify-between bg-bg border-2 rounded-xl ${conditionalClass()}`}>
			...
			<button type="button"
				onClick={copyMessage}
				className={`bg-bg-light px-2 py-2 rounded-xl hover:-motion-translate-y-loop-[10%] hover:motion-duration-700 ${copied ? "text-success" : "text-text-muted hover:text-text"}`}
			>
				{buttonIcon}
			</button>
		</div>
